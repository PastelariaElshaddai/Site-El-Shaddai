...
window.__ELSHADDAI_SUPABASE_ERROR__="Biblioteca do Supabase não carregada."; return;
}
...
const {data,error}=await window.supabaseClient.from("produtos").select("*").eq("ativo",true).order("id");
if(error){console.error(error);return []} return data||[];
}
async function carregarCategoriasSupabase(){
const {data,error}=await window.supabaseClient.from("categorias").select("*").eq("ativo",true).order("id");
if(error){console.error(error);return []} return data||[];
}
async function carregarConfiguracaoLojaSupabase(){
const {data,error}=await window.supabaseClient.from("configuracoes_loja").select("*").order("id",{ascending:true}).limit(1);
