...
function abrirTela(id){
  ...
  document.getElementById("tituloTela").textContent=id==="produtos"?"Produtos":id==="categorias"?"Categorias":id==="configuracao"?"Configuração da Loja":id==="pedidos"?"Pedidos":id==="clientes"?"Clientes":"Início";
  if(id==="produtos") renderProdutos();
  if(id==="categorias") renderCategorias();
  if(id==="configuracao" && typeof carregarConfiguracaoLoja==="function") carregarConfiguracaoLoja();
...
/* =========================
   BLOCO 6 — CLIENTES
   ========================= */
let clientes=[], pesquisaClientes="";

async function carregarClientes(){
  const box=$("listaClientes");
  if(!box) return;
  box.innerHTML="<div class='card muted'>Carregando clientes...</div>";
  try{
    ...
    box.innerHTML=`<div class='empty'>Não foi possível carregar os clientes.<br><small>${esc(err?.message||err)}</small></div>`;
  }
}

function aplicarPesquisaClientes(){
  pesquisaClientes=($("pesquisaClientes")?.value||"").trim().toLowerCase();
  renderClientes();
}

function atualizarResumoClientes(){
  const pedidos=clientes.reduce((s,c)=>s+Number(c.quantidade_pedidos||0),0);
  const gasto=clientes.reduce((s,c)=>s+Number(c.total_gasto||0),0);
  if($("clientesTotal")) $("clientesTotal").textContent=clientes.length;
  if($("clientesPedidos")) $("clientesPedidos").textContent=pedidos;
  if($("clientesGasto")) $("clientesGasto").textContent=moeda(gasto);
}
...
function renderClientes(){
  ...
  const lista=clientes.filter(c=>{
    const texto=[c.nome,c.telefone,c.endereco].map(v=>String(v||"").toLowerCase()).join(" ");
    return !pesquisaClientes || texto.includes(pesquisaClientes);
  });
  if(!lista.length){
    box.innerHTML=`<div class='empty'>${clientes.length?"Nenhum cliente encontrado.":"Nenhum cliente cadastrado ainda."}</div>`;
    return;
  }
  box.innerHTML=lista.map(c=>`
    <article class="client-card">
      <div class="client-head">
        <div>
          <div class="client-name">${esc(c.nome||"Cliente sem nome")}</div>
          <div class="client-meta">📱 ${esc(c.telefone||"Telefone não informado")}</div>
        </div>
        <span class="badge on">Cliente</span>
      </div>
      <div class="client-grid">
        <div class="client-box"><strong>Pedidos</strong>${Number(c.quantidade_pedidos||0)}</div>
        <div class="client-box"><strong>Total gasto</strong>${moeda(c.total_gasto)}</div>
        <div class="client-box"><strong>Endereço</strong>${esc(c.endereco||"Não informado")}</div>
      </div>
      <div class="client-actions">
        ${c.telefone?`<a class="btn primary" href="https://wa.me/${normalizarWhatsApp(c.telefone)}" target="_blank" rel="noopener">💬 WhatsApp</a>`:""}
      </div>
    </article>`).join("");
}
