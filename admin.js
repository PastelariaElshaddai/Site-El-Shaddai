...
async function salvarProduto(){const nome=$("nomeProduto").value.trim(),temTamanhos=!!$("produtoTemTamanhos")?.checked,categoria=$("categoriaProduto").value;const valorPreco=$("precoProduto").value.trim();const preco=temTamanhos?null:Number(valorPreco);if(!nome||!categoria)return alert("Preencha nome e categoria.");if(!temTamanhos&&(!valorPreco||!Number.isFinite(preco)||preco<0))return alert("Informe o preço do produto.");if(temTamanhos&&!tamanhosAtual.length)return alert("Cadastre pelo menos um tamanho e seu preço.");const payload={nome,preco,categoria,descricao:$("descricaoProduto").value.trim(),ativo:$("produtoAtivo").checked,disponivel:$("produtoDisponivel").checked,foto:fotoAtual||null,ingredientes:ingredientesAtual,adicionais:adicionalAtual,tamanhos:temTamanhos?tamanhosAtual:[]};const id=$("produtoId").value;const r=id?await supabaseClient.from("produtos").update(payload).eq("id",id):await supabaseClient.from("produtos").insert(payload);if(r.error)return alert("Não foi possível salvar o produto.\n\n"+r.error.message);cancelarProduto();await carregarTudoCompleto()}
