  ...
  if(foto){
    foto.addEventListener("change",e=>{
      ...
      if(file.size>1200000){
        alert("Escolha uma foto de até 1,2 MB.");
        e.target.value="";
        return;
      }
...
async function carregarTudo(){
  ...
  try{
    ...
    const timeout=new Promise((_,reject)=>setTimeout(()=>reject(new Error("Tempo limite de conexão com o Supabase.")),12000));
    const [c,p]=await Promise.race([consulta,timeout]);
    ...
    if(box) box.innerHTML="<div class='empty'>Não foi possível carregar os produtos.</div>";
    const cat=$("listaCategorias");
    if(cat) cat.innerHTML="<div class='empty'>Não foi possível carregar as categorias.</div>";
