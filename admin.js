...
async function lerImagem(input,cb){
  const f=input.files?.[0];
  if(!f)return;
  const LIMITE=1700000;
  const lerOriginal=()=>{const r=new FileReader();r.onload=()=>cb(r.result);r.readAsDataURL(f)};
  if(f.size<=LIMITE){lerOriginal();return}
  try{
    const original=await new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(r.result);r.onerror=reject;r.readAsDataURL(f)});
    const img=await new Promise((resolve,reject)=>{const im=new Image();im.onload=()=>resolve(im);im.onerror=reject;im.src=original});
    const maxSide=1600;
    ...
    let qualidade=0.84,saida=canvas.toDataURL("image/jpeg",qualidade);
    while(saida.length*0.75>LIMITE && qualidade>0.45){qualidade-=0.07;saida=canvas.toDataURL("image/jpeg",qualidade)}
    if(saida.length*0.75>LIMITE){
      const escala2=0.75;
      canvas.width=Math.max(1,Math.round(canvas.width*escala2));
      canvas.height=Math.max(1,Math.round(canvas.height*escala2));
      ctx.drawImage(img,0,0,canvas.width,canvas.height);
      saida=canvas.toDataURL("image/jpeg",0.72);
    }
    cb(saida);
  }catch(e){
    console.warn("Não foi possível otimizar a imagem; tentando original.",e);
    lerOriginal();
  }
}
...
function editarProduto(id){
  ...
  mostrarImagem("previewFoto",fotoAtual);
  tamanhosAtual=arr(p.tamanhos).map(x=>({nome:String(x.nome||""),preco:Number(x.preco||0)}));
  $("precoProduto").value=tamanhosAtual.length?"":(p.preco??"");
  const temT=$("produtoTemTamanhos");
  if(temT)temT.checked=tamanhosAtual.length>0;
  renderizarTamanhosAdmin();
  alternarTamanhosProduto();
  ingredientesAtual=arr(p.ingredientes).map(x=>typeof x==="string"?{nome:x,podeRetirar:true}:{nome:x.nome||"",podeRetirar:x.podeRetirar!==false});
  adicionalAtual=arr(p.adicionais).map(x=>({nome:x.nome,preco:Number(x.preco||0)}));
  renderIngredientes();
  renderAdicionais();
...
function alternarTamanhosProduto(){
  ...
  const preco=$("precoProduto");
  if(editor)editor.style.display=ativo?"block":"none";
  if(preco){
    preco.disabled=!!ativo;
    preco.placeholder=ativo?"Preço definido pelos tamanhos":"Preço do produto";
    if(ativo)preco.value="";
  }
...
async function salvarProduto(){
  ...
  const temTamanhos=$("produtoTemTamanhos")?.checked===true;
  const precoBase=Number($("precoProduto").value);

  if(!nome||!categoria)return alert("Preencha nome e categoria.");

  if(temTamanhos){
    if(!tamanhosAtual.length)return alert("Cadastre pelo menos um tamanho com preço.");
  }else{
    if(!Number.isFinite(precoBase)||precoBase<0)return alert("Informe o preço do produto.");
  }

  const payload={
    nome,
    preco:temTamanhos?0:precoBase,
    categoria,
    ...
    tamanhos:temTamanhos?tamanhosAtual:[]
  };

  const id=$("produtoId").value;
  const r=id?await supabaseClient.from("produtos").update(payload).eq("id",id):await supabaseClient.from("produtos").insert(payload);
  if(r.error)return alert("Não foi possível salvar o produto.\n\n"+r.error.message);
  cancelarProduto();
  await carregarTudoCompleto();
}
...
function renderProdutos(){const termo=pesquisaProdutos,box=$("listaProdutos");if(!box)return;const f=produtos.filter(p=>(!categoriaFiltro||p.categoria===categoriaFiltro)&&(!termo||[p.nome,p.categoria,p.descricao].join(" ").toLowerCase().includes(termo)));box.innerHTML=f.length?f.map(p=>`<article class="product">${p.foto?`<img src="${esc(p.foto)}" alt="">`:`<div style="width:78px;height:78px;border-radius:12px;background:#eee;display:grid;place-items:center">Sem foto</div>`}<div><h3>${esc(p.nome)} <span class="badge ${p.ativo!==false?"on":"off"}">${p.ativo!==false?"Ativo":"Inativo"}</span> <span class="badge ${p.disponivel!==false?"on":"off"}">${p.disponivel!==false?"Disponível":"Indisponível"}</span></h3><p>${esc(p.categoria||"Sem categoria")}</p><p>${esc(p.descricao||"Sem descrição")}</p>${arr(p.tamanhos).length?"":`<p class="price">${moeda(p.preco)}</p>`}${arr(p.tamanhos).length?`<p><strong>Tamanhos:</strong> ${arr(p.tamanhos).map(t=>`${esc(t.nome)} — ${moeda(t.preco)}`).join(" · ")}</p>`:""}</div><div class="actions"><button class="btn light" onclick="editarProduto('${String(p.id)}')">Editar</button><button class="btn danger" onclick="excluirProduto('${String(p.id)}')">Excluir</button></div></article>`).join(""):"<div class='empty'>Nenhum produto encontrado.</div>"}



function novaCategoria(){$("categoriaEditId").value="";$("nomeCategoria").value="";$("descricaoCategoria").value="";$("categoriaAtiva").checked=true;abrirTela("categorias");$("nomeCategoria").focus()}
async function salvarCategoria(){const id=$("categoriaEditId").value,nome=$("nomeCategoria").value.trim(),descricao=$("descricaoCategoria").value.trim(),ativo=$("categoriaAtiva").checked;if(!nome)return alert("Informe o nome da categoria.");const antiga=id?categorias.find(c=>String(c.id)===String(id)):null;const dup=categorias.some(c=>String(c.nome||"").toLowerCase()===nome.toLowerCase()&&String(c.id)!==String(id));if(dup)return alert("Essa categoria já existe.");const r=id?await supabaseClient.from("categorias").update({nome,descricao,ativo}).eq("id",id):await supabaseClient.from("categorias").insert({nome,descricao,ativo});if(r.error)return alert(r.error.message);if(antiga&&antiga.nome!==nome){const rp=await supabaseClient.from("produtos").update({categoria:nome}).eq("categoria",antiga.nome);if(rp.error)return alert("A categoria foi renomeada, mas os produtos ligados a ela não puderam ser atualizados.\n\n"+rp.error.message)}novaCategoria();await carregarTudoCompleto()}
function editarCategoria(id){const c=categorias.find(x=>String(x.id)===String(id));if(!c)return;$("categoriaEditId").value=c.id;$("nomeCategoria").value=c.nome||"";$("descricaoCategoria").value=c.descricao||"";$("categoriaAtiva").checked=c.ativo!==false;abrirTela("categorias");$("nomeCategoria").focus()}
function abrirProdutosCategoria(id){const c=categorias.find(x=>String(x.id)===String(id));if(!c)return;categoriaFiltro=c.nome;abrirTela("produtos");const f=$("filtroCategoriaProdutos");if(f)f.value=c.nome;renderProdutos()}
async function excluirCategoria(id){const c=categorias.find(x=>String(x.id)===String(id));if(!c)return;if(produtos.some(p=>p.categoria===c.nome))return alert("Esta categoria possui produtos. Abra a pasta da categoria e mova/exclua os produtos antes de excluir a categoria.");if(!confirm("Excluir esta categoria?"))return;const r=await supabaseClient.from("categorias").delete().eq("id",id);if(r.error)return alert(r.error.message);await carregarTudoCompleto()}
function renderCategorias(){$("listaCategorias").innerHTML=categorias.length?categorias.map(c=>{const qtd=produtos.filter(p=>p.categoria===c.nome).length;return `<article class="card category-folder"><div class="category-folder-main"><h3>📁 ${esc(c.nome)} <span class="badge ${c.ativo!==false?"on":"off"}">${c.ativo!==false?"Ativa":"Inativa"}</span></h3><p>${esc(c.descricao||"Sem descrição")}</p><p><strong>${qtd}</strong> produto(s) nesta pasta.</p></div><div class="actions"><button class="btn blue" onclick="abrirProdutosCategoria('${c.id}')">Abrir produtos</button><button class="btn light" onclick="editarCategoria('${c.id}')">Editar nome</button><button class="btn danger" onclick="excluirCategoria('${c.id}')">Excluir</button></div></article>`}).join(""):"<div class='empty'>Nenhuma categoria cadastrada.</div>"}

function temaPadrao(){return {fundoPagina:"#fff8e1",destaque:"#ffb300",topo1:"#c62828",topo2:"#e53935",topo3:"#ffb300",textoTopo:"#ffffff",banner1:"#c62828",banner2:"#e53935",banner3:"#ffb300",textoBanner:"#ffffff",botao:"#ffb300",textoBotao:"#222222"}}
function lerInformacoesConfig(raw){const s=String(raw||"");const m4="__ELSHADDAI_CONFIG_V4__",m3="__ELSHADDAI_CONFIG_V3__";const marker=s.startsWith(m4)?m4:(s.startsWith(m3)?m3:null);if(!marker)return {informacoes:s,tema:temaPadrao(),atendimento:true,instagram:true,instagramUrl:""};try{const o=JSON.parse(s.slice(marker.length));return {informacoes:String(o.informacoes||""),tema:{...temaPadrao(),...(o.tema||{})},atendimento:o.atendimento!==false,instagram:o.instagram!==false,instagramUrl:String(o.instagramUrl||"")}}catch(e){return {informacoes:s.replace(/^__ELSHADDAI_CONFIG_V[34]__/ ,""),tema:temaPadrao(),atendimento:true,instagram:true,instagramUrl:""}}}
function salvarInformacoesConfig(informacoes,tema,atendimento,instagram,instagramUrl){return "__ELSHADDAI_CONFIG_V4__"+JSON.stringify({informacoes:String(informacoes||""),tema:{...temaPadrao(),...(tema||{})},atendimento:atendimento!==false,instagram:instagram!==false,instagramUrl:String(instagramUrl||"")})}
function valorTema(id,fallback){return ($(id)?.value||fallback)}
function obterTemaDoFormulario(){return {fundoPagina:valorTema("temaFundo","#fff8e1"),destaque:valorTema("temaDestaque","#ffb300"),topo1:valorTema("temaTopo1","#c62828"),topo2:valorTema("temaTopo2","#e53935"),topo3:valorTema("temaTopo3","#ffb300"),textoTopo:valorTema("temaTextoTopo","#ffffff"),banner1:valorTema("temaBanner1","#c62828"),banner2:valorTema("temaBanner2","#e53935"),banner3:valorTema("temaBanner3","#ffb300"),textoBanner:valorTema("temaTextoBanner","#ffffff"),botao:valorTema("temaBotao","#ffb300"),textoBotao:valorTema("temaTextoBotao","#222222")}}
function preencherTema(t){const x={...temaPadrao(),...(t||{})};const mapa={temaFundo:x.fundoPagina,temaDestaque:x.destaque,temaTopo1:x.topo1,temaTopo2:x.topo2,temaTopo3:x.topo3,temaTextoTopo:x.textoTopo,temaBanner1:x.banner1,temaBanner2:x.banner2,temaBanner3:x.banner3,temaTextoBanner:x.textoBanner,temaBotao:x.botao,temaTextoBotao:x.textoBotao};Object.entries(mapa).forEach(([id,v])=>{if($(id))$(id).value=v})}
function restaurarTemaPadrao(){preencherTema(temaPadrao())}
function aplicarConfigNosCampos(){const c=configLoja;if(!c)return;$('configNomeLoja').value=c.nome_loja||"";$('configSlogan').value=c.slogan||"";$('configWhatsapp').value=c.whatsapp||"";$('configPix').value=c.pix||"";const extra=lerInformacoesConfig(c.informacoes);$('configInformacoes').value=extra.informacoes;preencherTema(extra.tema);$('configLojaAberta').checked=c.loja_aberta!==false;$('configEntrega').checked=c.entrega_ativa!==false;$('configRetirada').checked=c.retirada_ativa!==false;$('configPagamentoPix').checked=c.pagamento_pix!==false;$('configPagamentoCartao').checked=c.pagamento_cartao!==false;$('configPagamentoDinheiro').checked=c.pagamento_dinheiro!==false;$('configLogoUrl').value=c.logo&&/^https?:\/\//.test(c.logo)?c.logo:"";$('configBannerUrl').value=c.banner&&/^https?:\/\//.test(c.banner)?c.banner:"";logoData=c.logo||null;bannerData=c.banner||null;mostrarImagem('configLogoPreview',logoData);mostrarImagem('configBannerPreview',bannerData)}
...
function lerMetaPromocao(p){try{const raw=String(p.descricao||"");const marker="__ELSHADAI_PROMO_V2__";if(!raw.startsWith(marker))return {tipo:"percentual",descricao:raw};return JSON.parse(raw.slice(marker.length))}catch(e){return {tipo:"percentual",descricao:String(p.descricao||"")}}}
function descricaoPromocao(meta){return "__ELSHADAI_PROMO_V2__"+JSON.stringify(meta)}
function atualizarCamposPromocao(){const tipo=$("promoTipo").value;const qtd=tipo==="quantidade";$("promoQtdWrap").style.display=qtd?"block":"none";$("promoPremioTipoWrap").style.display=qtd?"block":"none";$("promoPremioNomeWrap").style.display=qtd?"block":"none";$("promoValorLabel").textContent=tipo==="percentual"?"Desconto (%)":tipo==="valor"?"Desconto (R$)":"Valor do prêmio / desconto (opcional)";$("promoDesconto").max=tipo==="percentual"?"100":""}
function novaPromocao(){
  $("promoCodigo").value="";
  $("promoTipo").value="percentual";
  $("promoDesconto").value="";
  $("promoQuantidade").value="";
  $("promoPremioTipo").value="produto";
  $("promoPremioNome").value="";
  $("promoDescricao").value="";
  if($("promoPrimeiraCompra")) $("promoPrimeiraCompra").checked=false;
  if($("promoDestaque")) $("promoDestaque").checked=false;
  $("promoValidade").value="";
  $("promoAtivo").checked=true;
  $("promoCodigo").dataset.editId="";
  atualizarCamposPromocao()
}
...
function editarPromocao(id){
  const p=promocoes.find(x=>String(x.id)===String(id));
  if(!p)return;
  const m=lerMetaPromocao(p);
  if(m.primeiraCompra===undefined)m.primeiraCompra=/primeir[oa]/i.test(String(m.descricao||p.descricao||""))&&/pastel|compra/i.test(String(m.descricao||p.descricao||""));
  $("promoCodigo").value=p.codigo||"";
  $("promoCodigo").dataset.editId=p.id;
  $("promoTipo").value=m.tipo||"percentual";
  $("promoDesconto").value=p.desconto??"";
  $("promoQuantidade").value=m.quantidade||"";
  $("promoPremioTipo").value=m.premioTipo||"produto";
  $("promoPremioNome").value=m.premioNome||"";
  $("promoDescricao").value=m.descricao||"";
  if($("promoPrimeiraCompra")) $("promoPrimeiraCompra").checked=!!m.primeiraCompra;
  if($("promoDestaque")) $("promoDestaque").checked=!!m.destaque;
  $("promoValidade").value=p.validade||"";
  $("promoAtivo").checked=p.ativo!==false;
  atualizarCamposPromocao();
  abrirTela("promocoes");
  window.scrollTo({top:0,behavior:"smooth"})
}
async function salvarPromocao(){
  const codigo=$("promoCodigo").value.trim().toUpperCase(),
    tipo=$("promoTipo").value,
    desconto=Number($("promoDesconto").value||0),
    quantidade=Math.max(0,Number($("promoQuantidade").value||0)),
    premioTipo=$("promoPremioTipo").value,
    premioNome=$("promoPremioNome").value.trim(),
    descricao=$("promoDescricao").value.trim(),
    validade=$("promoValidade").value||null,
    ativo=$("promoAtivo").checked,
    primeiraCompra=!!$("promoPrimeiraCompra")?.checked,
    destaque=!!$("promoDestaque")?.checked;
  if(!codigo)return alert("Informe o código da promoção.");
  if(!Number.isFinite(desconto)||desconto<0||(tipo==="percentual"&&desconto>100))return alert(tipo==="percentual"?"A porcentagem deve ficar entre 0 e 100.":"Informe um valor válido.");
  if(tipo==="quantidade"&&quantidade<1)return alert("Informe pelo menos 1 compra para a regra.");
  if(tipo==="quantidade"&&!premioNome)return alert("Informe o nome do produto, combo, categoria ou prêmio.");
  const meta={tipo,quantidade:tipo==="quantidade"?quantidade:0,premioTipo:tipo==="quantidade"?premioTipo:"",premioNome:tipo==="quantidade"?premioNome:"",descricao,primeiraCompra,destaque};
  const payload={codigo,desconto,descricao:descricaoPromocao(meta),validade,ativo};
  const editId=$("promoCodigo").dataset.editId||"";
  const r=editId?await supabaseClient.from("promocoes").update(payload).eq("id",editId):await supabaseClient.from("promocoes").insert(payload);
  if(r.error)return alert("Não foi possível salvar a promoção.\n\n"+r.error.message);
  novaPromocao();
...
function textoTipoPromo(m,p){if(m.tipo==="percentual")return Number(p.desconto||0)+"% de desconto";if(m.tipo==="valor")return moeda(p.desconto)+" de desconto";return "A cada "+m.quantidade+" compra(s): "+esc(m.premioTipo||"prêmio")+" — "+esc(m.premioNome||"não informado")}
function renderPromocoes(){$("listaPromocoes").innerHTML=promocoes.length?promocoes.map(p=>{const m=lerMetaPromocao(p);const regras=[m.primeiraCompra?"Somente primeira compra":"",m.destaque?"Destaque no banner":""].filter(Boolean).join(" · ");return `<div class="coupon"><span><strong>${esc(p.codigo)}</strong><br>${textoTipoPromo(m,p)}<div class="promo-meta">${esc(m.descricao||"")}<br>Validade: ${esc(p.validade||"Sem data")} · ${p.ativo!==false?"Ativa":"Inativa"}${regras?" · "+esc(regras):""}</div></span><div class="actions"><button class="btn light" onclick="editarPromocao('${p.id}')">Editar</button><button class="btn danger" onclick="excluirPromocao('${p.id}')">Excluir</button></div></div>`}).join(""):"<div class='empty'>Nenhuma promoção cadastrada.</div>"}
...
function renderFidelidade(){const q=($('pesquisaFidelidade').value||'').toLowerCase(),filtro=$("filtroFidelidade")?.value||"todos",m=metaFid();let f=clientes.filter(c=>[c.nome,c.telefone].join(" ").toLowerCase().includes(q));if(filtro==="fiéis")f=f.filter(c=>Number(c.quantidade_pedidos||0)>=m);if(filtro==="progresso")f=f.filter(c=>Number(c.quantidade_pedidos||0)<m);f.sort((a,b)=>Number(b.quantidade_pedidos||0)-Number(a.quantidade_pedidos||0));$("fidClientes").textContent=clientes.length;$("fidLoyais").textContent=clientes.filter(c=>Number(c.quantidade_pedidos||0)>=m).length;$("fidCompras").textContent=clientes.reduce((s,c)=>s+Number(c.quantidade_pedidos||0),0);$("fidBeneficios").textContent=clientes.reduce((s,c)=>s+Math.floor(Number(c.quantidade_pedidos||0)/m),0);$("listaFidelidade").innerHTML=f.length?f.map(c=>{const n=Number(c.quantidade_pedidos||0),b=Math.floor(n/m),rest=n%m===0?m:m-(n%m),percent=Math.min(100,Math.round((n%m)/m*100)),fiel=n>=m;return `<div class="fid-card"><div class="fid-main"><h3>${esc(c.nome||"Cliente")} ${fiel?'<span class="badge on">Cliente fiel</span>':''}</h3><div>${esc(c.telefone||"Sem telefone")} · <strong>${n}</strong> compra(s) · ${moeda(c.total_gasto||0)}</div><div class="fid-progress"><span style="width:${percent}%"></span></div><div class="promo-meta">${fiel?b+" benefício(s) conquistado(s) no ciclo total · ":""}${n%m===0&&n>0?"Meta alcançada — novo ciclo disponível.":"Faltam "+rest+" compra(s) para o próximo benefício."}</div></div><div><strong>${b}</strong><br><small>benefício(s)</small></div></div>`}).join(""):"<div class='empty'>Nenhum cliente encontrado para este filtro.</div>"}
