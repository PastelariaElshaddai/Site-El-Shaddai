...
function novaPromocao(){$("promoCodigo").value="";$("promoTipo").value="percentual";$("promoDesconto").value="";$("promoQuantidade").value="";$("promoPremioTipo").value="produto";$("promoPremioNome").value="";$("promoDescricao").value="";$("promoValidade").value="";$("promoAtivo").checked=true;$("promoCodigo").dataset.editId="";atualizarCamposPromocao()}
...
function editarPromocao(id){const p=promocoes.find(x=>String(x.id)===String(id));if(!p)return;const m=lerMetaPromocao(p);$("promoCodigo").value=p.codigo||"";$("promoCodigo").dataset.editId=p.id;$("promoTipo").value=m.tipo||"percentual";$("promoDesconto").value=p.desconto??"";$("promoQuantidade").value=m.quantidade||"";$("promoPremioTipo").value=m.premioTipo||"produto";$("promoPremioNome").value=m.premioNome||"";$("promoDescricao").value=m.descricao||"";$("promoValidade").value=p.validade||"";$("promoAtivo").checked=p.ativo!==false;atualizarCamposPromocao();abrirTela("promocoes");window.scrollTo({top:0,behavior:"smooth"})}
async function salvarPromocao(){const codigo=$("promoCodigo").value.trim().toUpperCase(),tipo=$("promoTipo").value,desconto=Number($("promoDesconto").value||0),quantidade=Math.max(0,Number($("promoQuantidade").value||0)),premioTipo=$("promoPremioTipo").value,premioNome=$("promoPremioNome").value.trim(),descricao=$("promoDescricao").value.trim(),validade=$("promoValidade").value||null,ativo=$("promoAtivo").checked;if(!codigo)return alert("Informe o código da promoção.");if(!Number.isFinite(desconto)||desconto<0||(tipo==="percentual"&&desconto>100))return alert(tipo==="percentual"?"A porcentagem deve ficar entre 0 e 100.":"Informe um valor válido.");if(tipo==="quantidade"&&quantidade<1)return alert("Informe pelo menos 1 compra para a regra.");if(tipo==="quantidade"&&!premioNome)return alert("Informe o nome do produto, combo, categoria ou prêmio.");const meta={tipo,quantidade:tipo==="quantidade"?quantidade:0,premioTipo:tipo==="quantidade"?premioTipo:"",premioNome:tipo==="quantidade"?premioNome:"",descricao};const payload={codigo,desconto,descricao:descricaoPromocao(meta),validade,ativo};const editId=$("promoCodigo").dataset.editId||"";const r=editId?await supabaseClient.from("promocoes").update(payload).eq("id",editId):await supabaseClient.from("promocoes").insert(payload);if(r.error)return alert("Não foi possível salvar a promoção.\n\n"+r.error.message);novaPromocao();await carregarPromocoes(true)}
