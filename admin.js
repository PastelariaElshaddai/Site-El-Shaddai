...
/* BLOCO 7 — DASHBOARD / INÍCIO */
async function carregarDashboard(){
  try{
    const hoje=new Date();
    const inicio=new Date(hoje.getFullYear(),hoje.getMonth(),hoje.getDate()).toISOString();
    const amanha=new Date(hoje.getFullYear(),hoje.getMonth(),hoje.getDate()+1).toISOString();
    const [pr,cr]=await Promise.all([
      supabaseClient.from("pedidos").select("id,cliente_nome,total,status,criado_em,itens").gte("criado_em",inicio).lt("criado_em",amanha).order("criado_em",{ascending:false}),
      supabaseClient.from("clientes").select("id")
    ]);
    if(pr.error) throw pr.error;
    if(cr.error) throw cr.error;
    const pedidos=pr.data||[], clientes=cr.data||[];
    const faturamento=pedidos.reduce((s,p)=>s+(Number(p.total)||0),0);
    let vendidos=0;
    pedidos.forEach(p=>{if(Array.isArray(p.itens))p.itens.forEach(i=>vendidos+=Number(i.quantidade??i.qtd??1)||0)});
    const fmt=v=>Number(v||0).toLocaleString("pt-BR",{style:"currency",currency:"BRL"});
    const el=id=>document.getElementById(id);
    if(el("dashPedidosHoje"))el("dashPedidosHoje").textContent=pedidos.length;
    if(el("dashFaturamentoHoje"))el("dashFaturamentoHoje").textContent=fmt(faturamento);
    if(el("dashClientes"))el("dashClientes").textContent=clientes.length;
    if(el("dashProdutosVendidos"))el("dashProdutosVendidos").textContent=vendidos;
    const novos=pedidos.filter(p=>!p.status||p.status==="novo"||p.status==="pendente");
    if(el("dashPedidosNovos"))el("dashPedidosNovos").innerHTML=novos.length?novos.slice(0,8).map(p=>`<div class="dashboard-item"><strong>Pedido #${p.id} — ${escAttr(p.cliente_nome||"Cliente")}</strong><small>Total: ${fmt(p.total)}</small></div>`).join(""):'<p class="dashboard-vazio">Nenhum pedido aguardando atendimento.</p>';
    if(el("dashResumoVendas"))el("dashResumoVendas").innerHTML=`<div class="dashboard-item"><strong>📦 Pedidos</strong><small>${pedidos.length} pedido(s) hoje</small></div><div class="dashboard-item"><strong>💰 Faturamento</strong><small>${fmt(faturamento)}</small></div><div class="dashboard-item"><strong>🥟 Produtos vendidos</strong><small>${vendidos} unidade(s)</small></div>`;
