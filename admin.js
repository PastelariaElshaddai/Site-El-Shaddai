...
async function salvarProduto(){const nome=$("nomeProduto").value.trim(),temT=!!$("produtoTemTamanhos")?.checked,categoria=$("categoriaProduto").value,precoBase=Number($("precoProduto").value);if(!nome||!categoria)return alert("Preencha nome e categoria.");if(temT&&!tamanhosAtual.length)return alert("Cadastre pelo menos um tamanho.");if(!temT&&(!Number.isFinite(precoBase)||precoBase<0))return alert("Informe o preço do produto.");const payload={nome,preco:temT?0:precoBase,categoria,descricao:$("descricaoProduto").value.trim(),ativo:$("produtoAtivo").checked,disponivel:$("produtoDisponivel").checked,foto:fotoAtual||null,ingredientes:ingredientesAtual,adicionais:adicionalAtual,tamanhos:temT?tamanhosAtual:[]};const id=$("produtoId").value;const r=id?await supabaseClient.from("produtos").update(payload).eq("id",id):await supabaseClient.from("produtos").insert(payload);if(r.error)return alert("Não foi possível salvar o produto.\n\n"+r.error.message);cancelarProduto();await carregarTudoCompleto()}
...
async function salvarCategoria(){const id=$("categoriaEditId").value,nome=$("nomeCategoria").value.trim(),descricao=$("descricaoCategoria").value.trim(),ativo=$("categoriaAtiva").checked;if(!nome)return alert("Informe o nome da categoria.");const dup=categorias.some(c=>c.nome.toLowerCase()===nome.toLowerCase()&&String(c.id)!==String(id));if(dup)return alert("Essa categoria já existe.");const antigo=id?categorias.find(c=>String(c.id)===String(id)):null;const r=id?await supabaseClient.from("categorias").update({nome,descricao,ativo}).eq("id",id):await supabaseClient.from("categorias").insert({nome,descricao,ativo});if(r.error)return alert(r.error.message);if(antigo&&antigo.nome!==nome){const up=await supabaseClient.from("produtos").update({categoria:nome}).eq("categoria",antigo.nome);if(up.error)return alert("Categoria salva, mas os produtos não puderam ser atualizados: "+up.error.message)}novaCategoria();await carregarTudoCompleto()}
...
async function atualizarStatusPedido(id,status){const atual=pedidos.find(p=>String(p.id)===String(id));if(!atual)return;const r=await supabaseClient.from("pedidos").update({status}).eq("id",id);if(r.error)return alert(r.error.message);if(status==="concluido"&&atual.status!=="concluido"){const q=await supabaseClient.from("clientes").select("id,quantidade_pedidos,total_gasto").eq("telefone",atual.cliente_telefone).limit(1);const c=q.data?.[0];if(c){const u=await supabaseClient.from("clientes").update({quantidade_pedidos:Number(c.quantidade_pedidos||0)+1,total_gasto:Number(c.total_gasto||0)+Number(atual.total||0)}).eq("id",c.id);if(u.error)return alert("Pedido concluído, mas a fidelidade não pôde ser atualizada: "+u.error.message)}}await carregarPedidos(true);await carregarClientes(true);await carregarFidelidade(true)}
...
function novaPromocao(){$("promoCodigo").value="";$ ("promoDesconto").value="";$ ("promoQuantidade").value="";$ ("promoPremioTipo").value="produto";$ ("promoPremioNome").value="";$ ("promoDescricao").value="";$ ("promoValidade").value="";$ ("promoAtivo").checked=true;$ ("promoTipo").value="percentual";atualizarCamposPromocao()}
function atualizarCamposPromocao(){const tipo=$("promoTipo").value;$("promoQtdWrap").style.display=tipo==="quantidade"?"block":"none";$("promoPremioTipoWrap").style.display=tipo==="quantidade"?"block":"none";$("promoPremioNomeWrap").style.display=tipo==="quantidade"?"block":"none";$("promoValorLabel").textContent=tipo==="percentual"?"Desconto (%)":tipo==="valor"?"Desconto (R$)":"Valor do desconto (opcional)"}
async function carregarPromocoes(silencioso=false){try{const r=await supabaseClient.from("promocoes").select("*").order("id",{ascending:false});if(r.error)throw r.error;promocoes=r.data||[];renderPromocoes()}catch(e){$("listaPromocoes").innerHTML=`<div class='notice'>A tabela de promoções não pôde ser consultada.</div>`;if(!silencioso)console.error(e)}}
async function salvarPromocao(){const codigo=$("promoCodigo").value.trim().toUpperCase(),tipo=$("promoTipo").value,desconto=Number($("promoDesconto").value||0),quantidade=Number($("promoQuantidade").value||0),premioTipo=$("promoPremioTipo").value,premioNome=$("promoPremioNome").value.trim(),descricaoBase=$("promoDescricao").value.trim(),validade=$("promoValidade").value||null,ativo=$("promoAtivo").checked;if(!codigo)return alert("Informe o código.");if(tipo==="percentual"&&(desconto<0||desconto>100))return alert("A porcentagem deve ficar entre 0 e 100.");if(tipo==="valor"&&desconto<0)return alert("Informe um valor válido.");if(tipo==="quantidade"&&(!Number.isInteger(quantidade)||quantidade<1))return alert("Informe a quantidade de compras.");if(tipo==="quantidade"&&!premioNome)return alert("Informe o nome do prêmio.");const meta={_tipo:tipo,quantidade:tipo==="quantidade"?quantidade:null,premioTipo:tipo==="quantidade"?premioTipo:null,premioNome:tipo==="quantidade"?premioNome:null};const descricao="[PROMO_META]"+JSON.stringify(meta)+"[/PROMO_META]"+(descricaoBase?" "+descricaoBase:"");const r=await supabaseClient.from("promocoes").insert({codigo,desconto,descricao,validade,ativo});if(r.error)return alert("Não foi possível salvar a promoção.\n\n"+r.error.message);novaPromocao();await carregarPromocoes(true)}
...
function renderPromocoes(){$("listaPromocoes").innerHTML=promocoes.length?promocoes.map(p=>{const d=String(p.descricao||""),m=d.match(/\[PROMO_META\](.*?)\[\/PROMO_META\]/);let meta={_tipo:"percentual"};try{if(m)meta={...meta,...JSON.parse(m[1])}}catch(e){}const resumo=meta._tipo==="quantidade"?`${meta.quantidade} compra(s) → ${esc(meta.premioTipo)}: ${esc(meta.premioNome)}`:meta._tipo==="valor"?moeda(p.desconto)+" de desconto":Number(p.desconto||0)+"% de desconto";return `<div class="coupon"><span><strong>${esc(p.codigo)}</strong><br>${resumo}<br><small>Validade: ${esc(p.validade||"Sem data")} · ${p.ativo!==false?"Ativa":"Inativa"}</small></span><button class="btn danger" onclick="excluirPromocao('${p.id}')">Excluir</button></div>`}).join(""):"<div class='empty'>Nenhuma promoção cadastrada.</div>"}
...
function renderFidelidade(){const q=($("pesquisaFidelidade").value||"").toLowerCase(),m=metaFid(),filtro=$("filtroFidelidade")?.value||"todos";let f=clientes.filter(c=>[c.nome,c.telefone].join(" ").toLowerCase().includes(q));if(filtro==="fiéis")f=f.filter(c=>Number(c.quantidade_pedidos||0)>=m);if(filtro==="progresso")f=f.filter(c=>Number(c.quantidade_pedidos||0)<m);$("fidClientes").textContent=clientes.length;$("fidLoyais").textContent=clientes.filter(c=>Number(c.quantidade_pedidos||0)>=m).length;$("fidCompras").textContent=clientes.reduce((s,c)=>s+Number(c.quantidade_pedidos||0),0);$("fidBeneficios").textContent=clientes.reduce((s,c)=>s+Math.floor(Number(c.quantidade_pedidos||0)/m),0);$("listaFidelidade").innerHTML=f.length?f.map(c=>{const n=Number(c.quantidade_pedidos||0),rest=Math.max(0,m-(n%m)),b=Math.floor(n/m),fiel=n>=m;return `<div class="rank"><span><strong>${esc(c.nome||"Cliente")}</strong> ${fiel?'<span class="badge on">Cliente fiel</span>':''}<br>${esc(c.telefone||"")}<br>${n} compra(s) · ${moeda(c.total_gasto)}</span><span>${b>0?b+" benefício(s) conquistado(s)":"Faltam "+rest+" compra(s)"}</span></div>`}).join(""):"<div class='empty'>Nenhum cliente encontrado.</div>"}
