...
let pesquisaProdutos="", categoriaFiltro="";

/* =========================
   BLOCO 3/4 — CONFIGURAÇÃO DA LOJA
   ========================= */

let configuracaoLoja=null;

async function carregarConfiguracaoLoja(){
  const ajuda=$("configAjuda");
  if(ajuda) ajuda.textContent="Carregando configuração...";
  ...
  try{
    ...
    if(!r.data){
      configuracaoLoja=null;
      preencherFormularioConfiguracao({});
      if(ajuda) ajuda.textContent="Nenhuma configuração encontrada.";
      return;
    }

    configuracaoLoja=r.data;
    preencherFormularioConfiguracao(r.data);

    if(ajuda) ajuda.textContent="Configuração carregada do Supabase.";
  }catch(err){
    console.error("Erro ao carregar configuração:",err);
    if(ajuda) ajuda.textContent="Erro ao carregar.";
    alert("Não foi possível carregar a configuração da loja.\n\n"+(err?.message||err));
  }
}

function preencherFormularioConfiguracao(c){
  $("configNomeLoja").value=c.nome_loja||"";
  $("configSlogan").value=c.slogan||"";
  $("configWhatsapp").value=c.whatsapp||"";
  $("configPix").value=c.pix||"";
  $("configLogo").value=c.logo||"";
  $("configBanner").value=c.banner||"";
  $("configInformacoes").value=c.informacoes||"";

  $("configLojaAberta").checked=c.loja_aberta!==false;
  $("configEntrega").checked=c.entrega_ativa!==false;
  $("configRetirada").checked=c.retirada_ativa!==false;
  $("configPagamentoPix").checked=c.pagamento_pix!==false;
  $("configPagamentoCartao").checked=c.pagamento_cartao!==false;
  $("configPagamentoDinheiro").checked=c.pagamento_dinheiro!==false;
}

async function salvarConfiguracaoLoja(){
  const nome_loja=$("configNomeLoja").value.trim();
  const slogan=$("configSlogan").value.trim();
  const whatsapp=$("configWhatsapp").value.trim();
  const pix=$("configPix").value.trim();
  const logo=$("configLogo").value.trim();
  const banner=$("configBanner").value.trim();
  const informacoes=$("configInformacoes").value.trim();
  ...
  const payload={
    ...
    banner,
    informacoes,
    loja_aberta:$("configLojaAberta").checked,
    entrega_ativa:$("configEntrega").checked,
    retirada_ativa:$("configRetirada").checked,
    pagamento_pix:$("configPagamentoPix").checked,
    pagamento_cartao:$("configPagamentoCartao").checked,
    pagamento_dinheiro:$("configPagamentoDinheiro").checked
  };
  ...
  try{
    ...
    if(configuracaoLoja?.id){
      r=await supabaseClient
        .from("configuracoes_loja")
        .update(payload)
        .eq("id",configuracaoLoja.id)
        .select()
        .single();
    }else{
      r=await supabaseClient
    ...
    if(ajuda) ajuda.textContent="Configuração salva com sucesso.";
    alert("Configuração da loja salva!");
  }catch(err){
    console.error("Erro ao salvar configuração:",err);
    $("status").textContent="● Erro";
    if(ajuda) ajuda.textContent="Erro ao salvar.";
...
async function iniciarPainel(){
  try{
    ...
    if(foto) foto.addEventListener("change",e=>{
      ...
      r.onload=()=>{fotoAtual=r.result;const preview=$("previewFoto");if(preview){preview.src=fotoAtual;preview.style.display="block"}};
      r.readAsDataURL(file);
    });
    await carregarTudo();
  }catch(err){
    console.error("Falha ao iniciar painel:",err);
    $("status").textContent="● Erro de inicialização";
    alert("O painel não conseguiu iniciar.\n\n"+(err?.message||err));
  }
}
if(document.readyState==="loading") document.addEventListener("DOMContentLoaded",iniciarPainel);
else iniciarPainel();
...
function abrirTela(id){
  ...
  if(id==="configuracao") carregarConfiguracaoLoja();
}

async function carregarTudo(){
  $("status").textContent="● Conectando...";
  try{
    if(typeof window.supabase==="undefined")throw new Error("A biblioteca do Supabase não foi carregada.");
    if(typeof supabaseClient==="undefined"||!supabaseClient)throw new Error("O cliente do Supabase não foi criado.");
    ...
    const limite=new Promise((_,reject)=>setTimeout(()=>reject(new Error("Tempo limite: o Supabase não respondeu em até 12 segundos.")),12000));
    const [c,p]=await Promise.race([consulta,limite]);
    if(c.error)throw new Error("Categorias: "+c.error.message);
    if(p.error)throw new Error("Produtos: "+p.error.message);
    categorias=c.data||[];produtos=p.data||[];
    montarCatalogoAdicionais();
    $("status").textContent="● Conectado";
    popularCategorias();renderProdutos();renderCategorias();
  }catch(err){
    console.error("Erro no painel administrativo:",err);
    $("status").textContent="● Erro de conexão";
    const box=$("listaProdutos");if(box)box.innerHTML=`<div class="empty">Não foi possível carregar os produtos.<br><small>${esc(err?.message||err)}</small></div>`;
    const cat=$("listaCategorias");if(cat)cat.innerHTML=`<div class="empty">Não foi possível carregar as categorias.<br><small>${esc(err?.message||err)}</small></div>`;
    alert("O painel não conseguiu conectar ao Supabase.\n\n"+(err?.message||err));
  }
}
...
function prepararCategorias(){
  ...
  `;
  box.parentNode.insertBefore(painel, box);
...
function abrirCategoriaProdutos(nome){
  categoriaFiltro=nome||"";
  pesquisaProdutos="";
  const campo=$("pesquisaProdutos");
...
function aplicarPesquisaProdutos(){
  const campo=$("pesquisaProdutos");
  pesquisaProdutos=(campo?.value||"").trim().toLowerCase();
  renderProdutos();
}
...
function renderProdutos(){
  ...
  if(!box)return;
  const termo=pesquisaProdutos;
  const filtrados=produtos.filter(p=>{
    const nomeCat=String(p.categoria||"");
    const bateCategoria=!categoriaFiltro || nomeCat===categoriaFiltro;
    const texto=[p.nome,p.categoria,p.descricao].map(v=>String(v||"").toLowerCase()).join(" ");
    return bateCategoria && (!termo || texto.includes(termo));
  });
  const titulo=$("filtroProdutosTitulo");
  if(titulo){
    titulo.textContent=categoriaFiltro?`Categoria: ${categoriaFiltro}`:"Todos os produtos";
  }
  const contador=$("contadorProdutos");
  if(contador){contador.textContent=`${filtrados.length} de ${produtos.length} produto(s)`;}
  if(!filtrados.length){
    box.innerHTML=`<div class='empty'>${produtos.length?"Nenhum produto encontrado com esse filtro.":"Nenhum produto cadastrado."}</div>`;
    return;
  }
...
function renderCategorias(){
  ...
  box.innerHTML=categorias.map(c=>{
    ...
    return `<article class="product" role="button" tabindex="0" onclick="abrirCategoriaProdutos('${esc(c.nome).replace(/'/g,"\\'")}')" onkeydown="if(event.key==='Enter'||event.key===' ')abrirCategoriaProdutos('${esc(c.nome).replace(/'/g,"\\'")}')" style="cursor:pointer">
      ...
      <div class="actions">
        <button class="btn light" type="button" onclick="event.stopPropagation();editarCategoria('${id}')">Editar</button>
        <button class="btn danger" type="button" onclick="event.stopPropagation();excluirCategoria('${id}')">Excluir</button>
      </div>
    </article>`;
  }).join("");
}
