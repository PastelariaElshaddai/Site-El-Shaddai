...
// Funciona tanto se o script carregar antes quanto depois do DOM estar pronto.
...
async function carregarTudo(){
  ...
  try{
    if(typeof supabaseClient === "undefined") throw new Error("Conexão com o Supabase não foi carregada.");
    ...
    if(box) box.innerHTML=`<div class="empty">Não foi possível carregar os dados agora.<br><small>${esc(e.message||e)}</small></div>`;
    const cat=$("listaCategorias");
    if(cat) cat.innerHTML=`<div class="empty">Não foi possível carregar as categorias.<br><small>${esc(e.message||e)}</small></div>`;
  }
}
