...
// ================= BLOCO 10 — WHATSAPP =================
const CHAVE_WHATSAPP_B10 = "elshaddai_whatsapp_config";
function obterWhatsAppBloco10(){
  try{ const c=JSON.parse(localStorage.getItem(CHAVE_WHATSAPP_B10)||"null"); if(c) return c; }catch(e){}
  return {numero:"5585988944421", resumo:true};
}
function carregarWhatsAppBloco10(){
  const c=obterWhatsAppBloco10();
  const n=document.getElementById("whatsappNumeroB10"), r=document.getElementById("whatsappResumoB10");
  if(n)n.value=c.numero||""; if(r)r.checked=c.resumo!==false;
}
function salvarWhatsAppBloco10(){
  const n=(document.getElementById("whatsappNumeroB10")?.value||"").replace(/\D/g,"");
  if(n.length<12){alert("Informe um número de WhatsApp válido com código do Brasil e DDD.");return;}
  const c={numero:n,resumo:document.getElementById("whatsappResumoB10")?.checked!==false};
  localStorage.setItem(CHAVE_WHATSAPP_B10,JSON.stringify(c)); alert("WhatsApp salvo com sucesso!");
}
function testarWhatsAppBloco10(){
  const c=obterWhatsAppBloco10();
  const n=(c.numero||"").replace(/\D/g,"");
  if(n.length<12){alert("Salve primeiro um número de WhatsApp válido.");return;}
  const msg="Olá! Este é um teste da integração do WhatsApp da Pastelaria El Shaddai.\n\n📲 O número está configurado corretamente.\n\nFeito a dois, no ponto pra você! ❤️";
  window.open("https://wa.me/"+n+"?text="+encodeURIComponent(msg),"_blank");
}
// garante a abertura da tela mesmo em versões anteriores da navegação
document.addEventListener("DOMContentLoaded",()=>{
  document.querySelectorAll('[data-screen="whatsapp"]').forEach(b=>b.addEventListener("click",()=>{setTimeout(carregarWhatsAppBloco10,0)}));
});
