...
function editarProduto(id){const p=produtos.find(x=>String(x.id)===String(id));if(!p)return;limparFormProduto();$("formTitulo").textContent="Editar produto";$("produtoId").value=p.id;$("nomeProduto").value=p.nome||"";$("categoriaProduto").value=p.categoria||"";$("descricaoProduto").value=p.descricao||"";$("produtoAtivo").checked=p.ativo!==false;$("produtoDisponivel").checked=p.disponivel!==false;fotoAtual=p.foto||null;mostrarImagem("previewFoto",fotoAtual);tamanhosAtual=arr(p.tamanhos).map(x=>({nome:String(x.nome||""),preco:Number(x.preco||0)}));const temT=$("produtoTemTamanhos");if(temT)temT.checked=tamanhosAtual.length>0;$("precoProduto").value=tamanhosAtual.length?"":(p.preco??"");alternarTamanhosProduto();renderizarTamanhosAdmin();ingredientesAtual=arr(p.ingredientes).map(x=>typeof x==="string"?{nome:x,podeRetirar:true}:{nome:x.nome||"",podeRetirar:x.podeRetirar!==false});adicionalAtual=arr(p.adicionais).map(x=>({nome:x.nome||"",preco:Number(x.preco||0)})).filter(x=>x.nome);renderIngredientes();renderAdicionais();$("formProduto").classList.add("open");}
function limparFormProduto(){
  ...
  const tt=$("produtoTemTamanhos");if(tt)tt.checked=false;const te=$("tamanhosEditor");if(te)te.style.display="none";renderizarTamanhosAdmin();alternarTamanhosProduto();
...
function alternarTamanhosProduto(){
  ...
  const precoCampo=$("precoProduto");
  const precoLabel=precoCampo?.closest(".field")?.querySelector(".title");
  if(editor)editor.style.display=ativo?"block":"none";
  if(precoCampo){
    precoCampo.disabled=!!ativo;
    precoCampo.required=!ativo;
    if(ativo){
      precoCampo.value="";
      precoCampo.placeholder="Não usado quando há tamanhos";
    }else{
      precoCampo.placeholder="Preço do produto";
    }
  }
  if(precoLabel)precoLabel.textContent=ativo?"Preço base (não usado com tamanhos)":"Preço";
...
async function salvarProduto(){const nome=$("nomeProduto").value.trim(),temTamanhos=!!$("produtoTemTamanhos")?.checked,categoria=$("categoriaProduto").value;const valorPreco=$("precoProduto").value.trim();const preco=temTamanhos?0:Number(valorPreco);if(!nome||!categoria)return alert("Preencha nome e categoria.");if(!temTamanhos&&(!valorPreco||!Number.isFinite(preco)||preco<0))return alert("Informe o preço do produto.");if(temTamanhos&&!tamanhosAtual.length)return alert("Cadastre pelo menos um tamanho e seu preço.");const payload={nome,preco,categoria,descricao:$("descricaoProduto").value.trim(),ativo:$("produtoAtivo").checked,disponivel:$("produtoDisponivel").checked,foto:fotoAtual||null,ingredientes:ingredientesAtual,adicionais:adicionalAtual,tamanhos:temTamanhos?tamanhosAtual:[]};const id=$("produtoId").value;const r=id?await supabaseClient.from("produtos").update(payload).eq("id",id):await supabaseClient.from("produtos").insert(payload);if(r.error)return alert("Não foi possível salvar o produto.\n\n"+r.error.message);cancelarProduto();await carregarTudoCompleto()}
