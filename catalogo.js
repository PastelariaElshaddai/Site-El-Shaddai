...
(function () {
  ...
  function aplicarContatos(c) {
    var wrap=document.getElementById("acoesContatoCliente");
    var bw=document.getElementById("btnWhatsCliente");
    var bi=document.getElementById("btnInstagramCliente");
    if(!wrap) return;
    var whatsapp=String(c&&c.whatsapp||"").replace(/\D/g,"");
    var waAtivo=c ? c.atendimento_ativo!==false : false;
    var ig=normalizarInstagram(c&&c.instagram);
    var igAtivo=c ? c.instagram_ativo!==false : false;
    if(bw){bw.href=whatsapp?"https://wa.me/"+whatsapp:"#";bw.style.display=waAtivo&&whatsapp?"flex":"none";}
    if(bi){bi.href=ig||"#";bi.style.display=igAtivo&&ig?"flex":"none";}
    wrap.style.display=(waAtivo&&whatsapp)||(igAtivo&&ig)?"flex":"none";
  }

  function limparDescricaoPromocao(v) {
    var t=String(v||"").trim();
    if(!t) return "";
    var marca="__ELSHADDAI_PROMO_V2__";
    if(t.indexOf(marca)===0){
      try {
        var o=JSON.parse(t.slice(marca.length));
        if(o && o.descricao) return String(o.descricao).trim();
      } catch(e) {
        var m=t.match(/"descricao"\s*:\s*"((?:\\.|[^"\\])*)"/);
  ...
  async function carregarPromocaoCliente() {
    try {
      var r=await supabaseClient.from("promocoes").select("id,codigo,desconto,descricao,validade,ativo,criado_em").eq("ativo",true).order("id",{ascending:false}).limit(1);
      if(r.error) throw r.error;
      var hoje=new Date().toISOString().slice(0,10);
      var p=(r.data||[]).find(function(x){return !x.validade || x.validade>=hoje;})||null;
      var box=document.querySelector(".banner-promocao");
      if(!box) return;
      if(!p){box.style.display="none";return;}
      box.style.display="block";
      var h=box.querySelector("h2"), q=box.querySelector("p");
      if(h) h.textContent="PROMOÇÃO DA SEMANA";
      if(q){
        var desc=limparDescricaoPromocao(p.descricao);
        q.textContent=desc || (Number(p.desconto||0)>0 ? "Desconto de "+Number(p.desconto||0)+"% com o cupom "+String(p.codigo||"") : "Confira nossa promoção atual.");
      }
    } catch(e) { console.error("Erro ao carregar promoção do cliente:",e); }
  }
