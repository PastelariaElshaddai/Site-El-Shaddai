    ...
    nomes.forEach(function(nomeCategoria){
      ...
      produtos.filter(function(p){
        ...
        card.innerHTML =
          ...
          (arr(produto.tamanhos).length
            ? '<p class="preco" style="font-size:14px">Escolha um tamanho</p>'
...
function escolherTamanhoProduto(nome,tamanhos,ingredientes,adicionais){
  ...
  var html="<h2 style='text-align:center;color:#c62828;margin-top:0'>Escolha o tamanho</h2><p style='text-align:center;font-weight:bold;font-size:18px'>"+escapar(nome)+"</p>";

  tamanhos.forEach(function(t,i){
    html+="<label style='display:flex;justify-content:space-between;align-items:center;gap:12px;background:white;border:2px solid #ddd;border-radius:12px;padding:14px;margin-bottom:9px;cursor:pointer;line-height:1.35'>"+
      "<span style='min-width:0;overflow-wrap:anywhere'><strong>"+escapar(t.nome)+"</strong> — <span style='color:#c62828;font-weight:bold'>"+dinheiro(t.preco)+"</span></span>"+
      "<input type='radio' name='tamanhoProduto' value='"+i+"' "+(i===0?"checked":"")+" style='width:22px;height:22px;flex:0 0 auto'>"+
      "</label>";
  });
  ...
  caixa.querySelector("#confirmarTamanhoProduto").onclick=function(){
    ...
    if(!r)return alert("Escolha um tamanho.");

    var t=tamanhos[Number(r.value)];
    fundo.remove();

    if(typeof personalizarProduto==="function"){
      personalizarProduto(nome+" - "+t.nome,Number(t.preco),ingredientes||[],adicionais||[]);
    }else{
      carrinho.push({
        nome:nome+" - "+t.nome,
        preco:Number(t.preco),
        quantidade:1,
        detalhes:["Tamanho: "+t.nome]
      });
