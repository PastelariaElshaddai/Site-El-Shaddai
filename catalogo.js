...
(function(){
  ...
  function lerInformacoesConfig(raw){const s=String(raw||"");const m4="__ELSHADDAI_CONFIG_V4__",m3="__ELSHADDAI_CONFIG_V3__";const marker=s.startsWith(m4)?m4:(s.startsWith(m3)?m3:null);if(!marker)return {informacoes:s,tema:temaPadrao(),atendimento:true,instagram:true,instagramUrl:""};try{const o=JSON.parse(s.slice(marker.length));return {informacoes:String(o.informacoes||""),tema:{...temaPadrao(),...(o.tema||{})},atendimento:o.atendimento!==false,instagram:o.instagram!==false,instagramUrl:String(o.instagramUrl||"")}}catch(e){return {informacoes:s,tema:temaPadrao(),atendimento:true,instagram:true,instagramUrl:""}}}
  function aplicarTema(t){const x={...temaPadrao(),...(t||{})};let st=document.getElementById("elshaddaiTemaDinamico");if(!st){st=document.createElement("style");st.id="elshaddaiTemaDinamico";document.head.appendChild(st)}st.textContent=`body{background:${x.fundoPagina}!important}header{background:linear-gradient(135deg,${x.topo1},${x.topo2},${x.topo3})!important;color:${x.textoTopo}!important}header h1,header p{color:${x.textoTopo}!important}.banner-principal{background:linear-gradient(135deg,${x.banner1},${x.banner2},${x.banner3})!important;color:${x.textoBanner}!important}.banner-principal strong{color:${x.textoBanner}!important}.botao,.botao-pesquisa{background:${x.botao}!important;color:${x.textoBotao}!important}.pesquisa:focus{border-color:${x.destaque}!important}.area-categorias{border-color:${x.destaque}!important}`}
  function aplicarConfiguracao(c){
    if(!c)return;
    const nome=c.nome_loja||"Pastelaria El Shaddai",slogan=c.slogan||"Feito a dois, no ponto pra você!",extra=lerInformacoesConfig(c.informacoes);
    const h1=document.querySelector("header h1"),p=document.querySelector("header p"),logo=document.querySelector(".logo-area img");
    if(h1)h1.textContent=nome;if(p)p.textContent=slogan;if(logo&&c.logo){logo.src=c.logo;logo.alt=nome}document.title=nome;aplicarTema(extra.tema);
    const banner=document.querySelector(".banner-principal");
    if(banner&&c.banner){const x={...temaPadrao(),...(extra.tema||{})};banner.style.backgroundImage="linear-gradient(135deg,"+x.banner1+", "+x.banner2+", "+x.banner3+"),url('"+String(c.banner).replace(/'/g,"%27")+"')";banner.style.backgroundSize="cover";banner.style.backgroundPosition="center"}
  }
  ...
  function renderProdutos(categorias,produtos){
    ...
    nomes.forEach(nomeCategoria=>{
      ...
      produtos.filter(p=>p&&p.categoria===nomeCategoria&&p.ativo!==false).forEach(produto=>{
        ...
        if(produto.disponivel===false){btn.disabled=true;btn.style.opacity=".55";btn.style.cursor="not-allowed"}else{btn.onclick=function(){const ing=ingredientesDo(produto),ad=adicionaisDo(produto),tam=arr(produto.tamanhos).map(t=>({nome:String(t.nome||""),preco:Number(t.preco||0)})).filter(t=>t.nome);if(tam.length){window.escolherTamanhoProduto(produto.nome,tam,ing,ad)}else{if(typeof window.personalizarProduto!=="function")window.personalizarProduto=personalizarProdutoFallback;window.personalizarProduto(produto.nome,Number(produto.preco||0),ing,ad)}}}
  ...
  function renderContato(c,extra){
    let box=document.getElementById("contatoLojaCliente");
    if(!box){box=document.createElement("div");box.id="contatoLojaCliente";box.style.cssText="display:flex;gap:10px;justify-content:center;flex-wrap:wrap;margin:18px 0 8px";const container=document.querySelector(".container");if(container)container.appendChild(box);}
    box.innerHTML="";
    if(extra.atendimento!==false && c.whatsapp){const n=String(c.whatsapp).replace(/\D/g,"");const a=document.createElement("a");a.href="https://wa.me/"+n+"?text="+encodeURIComponent("Olá, Pastelaria El Shaddai. Gostaria de falar com um atendente.");a.target="_blank";a.rel="noopener";a.textContent="Falar com atendente";a.style.cssText="padding:11px 16px;border:1px solid #ddd;border-radius:10px;text-decoration:none;font-weight:600";box.appendChild(a)}
    if(extra.instagram!==false && extra.instagramUrl){const a=document.createElement("a");a.href=extra.instagramUrl;a.target="_blank";a.rel="noopener";a.textContent="Instagram";a.style.cssText="padding:11px 16px;border:1px solid #ddd;border-radius:10px;text-decoration:none;font-weight:600";box.appendChild(a)}
  }
  ...
  async function sincronizar(){
    ...
    try{
      ...
      aplicarConfiguracao(config);const extra=lerInformacoesConfig(config&&config.informacoes);renderCategorias(categorias,produtos);renderContato(config||{},extra);renderProdutos(categorias,produtos);renderBannerPromocao(promocoes);await renderPromocoes(promocoes);
