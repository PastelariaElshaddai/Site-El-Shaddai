...
(function(){
  ...
  function interpretarPromo(p){
    const d=String(p?.descricao||""),marker="__ELSHADDAI_PROMO_V2__";
    if(d.startsWith(marker)){try{const meta=JSON.parse(d.slice(marker.length));if(meta.primeiraCompra===undefined)meta.primeiraCompra=/primeir[oa]/i.test(String(meta.descricao||d))&&/pastel|compra/i.test(String(meta.descricao||d));return {...p,...meta}}catch(e){}}
    const primeira=/(primeir[oa]\s+(?:compra|pastel))/i.test(d)||(/primeir[oa]/i.test(d)&&/pastel/i.test(d));
    return {...p,_tipo:"percentual",descricao:d,primeiraCompra:primeira,destaque:false}
  }
