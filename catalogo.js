...
(function () {
  ...
  function renderCategorias(categorias, produtos) {
    ...
    nomes.forEach(function(nome){
      ...
      b.onclick = function(){
        ...
        if (alvo) { alvo.scrollIntoView({behavior:"smooth", block:"start"}); return; }
        if (typeof window.irParaSecao === "function") window.irParaSecao(id);
      };
  ...
  function renderProdutos(categorias, produtos) {
    ...
    nomes.forEach(function(nomeCategoria){
      ...
      produtos.filter(function(p){
        ...
        if (btn) { btn.style.marginTop = "auto"; btn.style.width = "100%"; }
  ...
  function interpretarPromocaoCliente(p) {
    var d = String((p && p.descricao) || "");
    var m = d.match(/^__ELSHADAI_PROMO_V2__(.*)$/);
    if (!m) m = d.match(/\[PROMO_META\](.*?)\[\/PROMO_META\]/);
    if (!m) return Object.assign({}, p, {tipo:"percentual", descricao:d});
    try { return Object.assign({}, p, JSON.parse(m[1])); }
    catch(e) { return Object.assign({}, p, {tipo:"percentual", descricao:d}); }
  }

  function renderPromocoesCliente(promocoes) {
    var antigo = document.getElementById("promocoesCliente");
    if (antigo) antigo.remove();
    var container = document.querySelector(".container");
    if (!container) return;
    var agora = new Date();
    var lista = (promocoes || []).filter(function(p){
      if (!p || p.ativo === false) return false;
      if (p.validade && new Date(String(p.validade)+"T23:59:59") < agora) return false;
      return true;
    });
    if (!lista.length) return;
    var sec = document.createElement("section");
    sec.id = "promocoesCliente";
    sec.style.cssText = "margin:14px 0 20px;padding:16px;background:#fff3cd;border:2px solid #ffb300;border-radius:16px;box-shadow:0 3px 10px rgba(0,0,0,.08);";
    var titulo=document.createElement("h2"); titulo.textContent="🎁 Ofertas e cupons"; titulo.style.margin="0 0 10px"; sec.appendChild(titulo);
    lista.forEach(function(p){
      var m=interpretarPromocaoCliente(p), item=document.createElement("div");
      item.style.cssText="background:#fff;border:1px solid #ffd54f;border-radius:12px;padding:12px;margin-top:9px;";
      var beneficio = m.tipo === "percentual" ? (Number(p.desconto||0)+"% de desconto") : m.tipo === "valor" ? moeda(p.desconto)+" de desconto" : ("A cada "+Number(m.quantidade||0)+" compra(s): "+String(m.premioNome||"benefício"));
      item.innerHTML='<strong>'+esc(p.codigo||"Oferta")+'</strong><br><span>'+esc(beneficio)+'</span>'+(m.descricao?'<br><small>'+esc(m.descricao)+'</small>':'')+(p.validade?'<br><small>Válido até '+esc(p.validade)+'</small>':'');
      sec.appendChild(item);
    });
    var acoes=container.querySelector(".acoes");
    if (acoes) acoes.after(sec); else container.prepend(sec);
  }
  ...
  async function sincronizar() {
    ...
    try {
      ...
      renderPromocoesCliente(promocoes);
