  ...
  function renderProdutos(categorias, produtos) {
    ...
    nomes.forEach(function(nomeCategoria){
      ...
      produtos.filter(function(p){
        ...
        var tamanhosCard = arr(produto.tamanhos).map(function(t){
          return {nome:String(t.nome||""),preco:Number(t.preco||0)};
        }).filter(function(t){ return t.nome; });

        var precoHtml = tamanhosCard.length
          ? ""
...
function aplicarAjustesVisuaisPersonalizacao(){
  if(document.getElementById("elshaddaiPersonalizacaoStyle"))return;
  var st=document.createElement("style");st.id="elshaddaiPersonalizacaoStyle";st.textContent=`
    #personalizarModal label{gap:10px!important;max-width:100%!important;box-sizing:border-box!important;overflow:hidden!important;}
    #personalizarModal label>span{min-width:0!important;max-width:100%!important;overflow-wrap:anywhere!important;word-break:break-word!important;line-height:1.35!important;}
    #personalizarModal label input{flex:0 0 auto!important;}
    #personalizarModal #listaIngredientes,#personalizarModal #listaAdicionais{width:100%;max-width:100%;box-sizing:border-box;}
  `;document.head.appendChild(st);
}

(function(){
  aplicarAjustesVisuaisPersonalizacao();
  var obs=new MutationObserver(function(){if(document.getElementById("personalizarModal"))aplicarAjustesVisuaisPersonalizacao();});
...
function escolherTamanhoProduto(nome,tamanhos,ingredientes,adicionais){
  ...
  var caixa=document.createElement("div");caixa.style.cssText="width:100%;max-width:500px;background:#fff8e7;border:3px solid #ffb300;border-radius:20px;padding:20px;max-height:90vh;overflow:auto;box-sizing:border-box;";
  var escapeModal=function(v){return String(v??"").replace(/[&<>'"]/g,function(ch){return {"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':'&quot;'}[ch];});};
  var formatModalMoney=function(v){return Number(v||0).toLocaleString("pt-BR",{style:"currency",currency:"BRL"});};
  var html="<h2 style='text-align:center;color:#c62828;margin-top:0'>Escolha o tamanho</h2><p style='text-align:center;font-weight:bold;overflow-wrap:anywhere'>"+escapeModal(nome)+"</p>";
  tamanhos.forEach(function(t,i){html+="<label style='display:flex;justify-content:space-between;align-items:center;gap:10px;background:white;border:2px solid #ddd;border-radius:12px;padding:14px;margin-bottom:9px;cursor:pointer;overflow:hidden'><span style='min-width:0;overflow-wrap:anywhere;word-break:break-word'><strong>"+escapeModal(t.nome)+"</strong> — <span style='color:#c62828;font-weight:bold'>"+formatModalMoney(t.preco)+"</span></span><input type='radio' name='tamanhoProduto' value='"+i+"' "+(i===0?"checked":"")+" style='width:22px;height:22px;flex:0 0 auto'></label>";});
  html+="<button id='confirmarTamanhoProduto' style='width:100%;padding:15px;background:#ffb300;border:0;border-radius:12px;font-weight:bold;font-size:18px'>Continuar para personalizar</button><button id='fecharTamanhoProduto' style='width:100%;padding:13px;margin-top:9px;background:white;border:2px solid #c62828;border-radius:12px;color:#c62828;font-weight:bold'>Fechar</button>";
  ...
  caixa.querySelector("#confirmarTamanhoProduto").onclick=function(){
    ...
    fundo.remove();
    if(typeof personalizarProduto==="function") personalizarProduto(nome+" - "+t.nome,Number(t.preco||0),ingredientes||[],adicionais||[]);
    else if(typeof adicionarProduto==="function") adicionarProduto(nome+" - "+t.nome,Number(t.preco||0));
    else alert("Não foi possível abrir a personalização deste produto.");
  };
