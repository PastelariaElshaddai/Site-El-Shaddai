...
function escolherTamanhoProduto(nome,tamanhos,ingredientes,adicionais){
  ...
  var escapeModal=function(v){return String(v??"").replace(/[&<>'"]/g,function(c){return {"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;","":"&quot;"}[c];});};
  var formatModalMoney=function(v){return Number(v||0).toLocaleString("pt-BR",{style:"currency",currency:"BRL"});};
  var html="<h2 style='text-align:center;color:#c62828;margin-top:0'>Escolha o tamanho</h2><p style='text-align:center;font-weight:bold'>"+escapeModal(nome)+"</p>";
  tamanhos.forEach(function(t,i){html+="<label style='display:flex;justify-content:space-between;align-items:center;background:white;border:2px solid #ddd;border-radius:12px;padding:14px;margin-bottom:9px;cursor:pointer'><span><strong>"+escapeModal(t.nome)+"</strong> — <span style='color:#c62828;font-weight:bold'>"+formatModalMoney(t.preco)+"</span></span><input type='radio' name='tamanhoProduto' value='"+i+"' "+(i===0?"checked":"")+" style='width:22px;height:22px'></label>";});
